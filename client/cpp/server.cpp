...
Status statusHandler(KeyResponse response) {
  ...
  switch (error) {
    case AnnaError::NO_ERROR: std::cout << "Success!" << std::endl;
                             return Status::OK;
    case AnnaError::KEY_DNE: std::cout << "Not Found!" << std::endl;
                             return Status::NOT_FOUND;
    case AnnaError::WRONG_THREAD: std::cout << "Wrong Thread!" << std::endl;
                             return Status::INTERNAL;
    case AnnaError::NO_SERVERS: case AnnaError::TIMEOUT:
                             std::cout << "Unavailable!" << std::endl;
                             return Status::UNAVAILABLE;
    case AnnaError::LATTICE: std::cout << "Incorrect Lattice!" << std::endl;
                             return Status::INVALID_ARGUMENT;
    default: std::cout << "Unknown Failure!" << std::endl;
...
class LocalObjStoreImpl final : public ObjectStore::Service {
  ...
  Status CreateBucket(ServerContext* context,
    ...
    LWWPairLattice<std::set<string> > val(
        TimestampValuePair<std::set<string> >(generate_timestamp(0),
                  std::set::empty()));
  ...
  Status ListBucket(ServerContext* context,
    ...
    WWPairLattice<string> lww_lattice =
        deserialize_lww(response.tuples(0).payload());
    std::set filenames = lww_lattice.reveal().value;

    for (set<string>::iterator itr=filenames.begin();
  ...
  Status Get(ServerContext* context,
    ...
    WWPairLattice<string> lww_lattice =
        deserialize_lww(response.tuples(0).payload());
    std::set<string> filenames = lww_lattice.reveal().value;
    // check if the object exists in the bucket
    string filename = request->objectname();
    string objname = key + "/" + filename;
    if (filenames.find(objname) == std::end) {
      std::cout << "Object doesn't exist!" << std::end;
      return Status::NOT_FOUND;
    }
    // get the object
    KeyResponse response = get_request(this->getAnnaClient(), objname);
    Status status = statusHandler(response);
    if (status != Status::OK)
      return status;
    ...
    WWPairLattice<string> lww_lattice =
        deserialize_lww(response.tuples(0).payload());
    string data = lww_lattice.reveal().value;
    reply->aset_data(data);
    return status;
  }
  ...
  Status Put(ServerContext* context,
    ...
    WWPairLattice<string> lww_lattice =
        deserialize_lww(response.tuples(0).payload());
    std::set<string> filenames = lww_lattice.reveal().value;
    // add the objectname to the set
    string filename = request->objectname();
    string objname = key + "/" + filename;
    filenames.insert(objname);
    LWWPairLattice<std::set<string> > val(
        TimestampValuePair<std::set<string> >(generate_timestamp(0), filenames);
    Status status = put_request(this->getAnnaClient(), key, serialize(val))
  ...
  Status Delete(ServerContext* context,
    ...
    WWPairLattice<string> lww_lattice =
        deserialize_lww(response.tuples(0).payload());
    std::set<string> filenames = lww_lattice.reveal().value;
    // delete the objectname from the set
    string filename = request->objectname();
    string objname = key + "/" + filename;
